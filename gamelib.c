...
#define CINQUE_PUNTEGGI_PIU_ALTI 5
...
static void modificaSemeSelect(void);
extern void generaSeed(void);
static void selezionaSeed(void);
static int getRoomCount(void);

// 1 - Crea

static int selezionaPorta(void);
static int selezionaTipoStanza(void);
static int selezionaTrabocchetto(void);
static int selezionaTesoro(void);
...
static void creaStanzaMain(void);

// 2 - Cancella

static int cancellaStanza(void);
static void cancellaStanzaSelect(void);

// 3 - Stampa

static void stampaTipoStanza(Stanza *stanza);
static void stampaStanza(Stanza *stanza, int showAll);
static void stampaStanze(void);

// 4 - Genera

static int generaRandomPorte(int ultimaPorta);
static int generaRandomStanza(void);
static int generaRandomTrabocchetto(void);
static int generaRandomTesoro(void);
static void generaRandom(void);

// 5 - Chiudi

static int chiudiMappa(void);
...
static int inserisciNumeroGiocatori(void);
static Giocatore *creaGiocatore(void);
...
static int generaRandomNemico(void);

// gameover

static void mostraPunteggio(Giocatore *giocatore);

static void pausaEsecuzione(void);
...
Punteggio *tabellonePunteggi[CINQUE_PUNTEGGI_PIU_ALTI];
...
static void ordinaTabellonePunteggi(void)
{
    for (int i = 0; i < 5; i++)
    {
        for (int j = 1; j < 5 - i; j++)
...
{
    *semePtr = (unsigned int)rand() % 1000000;
    srand(*semePtr);
    return;
}
...
static Giocatore *popolaGiocatore(Giocatore *giocatore, int indice)
{
    ...
    while (giocatore->classe == NON_ASSEGNATA)
    {
...
static Stanza *creaStanzaSegreta(void)
{
